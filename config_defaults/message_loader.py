...
class MessageLoader:
    ...
    def _load_messages(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print("   Using minimal fallback messages.")
    ...
    def _get_fallback_messages(self) -> Dict[str, Any]:
        ...
        return {
            "emojis": {"error": "❌", "success": "✅", "warning": "⚠️", "info": "📊"},
            "errors": {
...
class CLIMessages:
    ...
    def ALERT_ITEM(emoji, ticker, alert_type, price):
        return CLIMessages._loader.get(
            "alerts.alert_item", emoji=emoji, ticker=ticker, alert_type=alert_type, price=price
        )
    ...
    def ALERT_HISTORY_ITEM(ticker, alert_type, time):
        return CLIMessages._loader.get(
            "alerts.history_item", ticker=ticker, alert_type=alert_type, time=time
        )
    ...
    def SCHEDULER_CONFIG(morning, evening, retries):
        return CLIMessages._loader.get(
            "scheduler.config", morning=morning, evening=evening, retries=retries
        )
    ...
    def SCHEDULER_HISTORY_ITEM(emoji, task, status, time):
        return CLIMessages._loader.get(
            "scheduler.history_item", emoji=emoji, task=task, status=status, time=time
        )
    ...
    def SCHEDULER_NEXT(task, time, hours, minutes):
        return CLIMessages._loader.get(
            "scheduler.next", task=task, time=time, hours=hours, minutes=minutes
        )
    ...
    def ACCOUNT_INFO(equity, cash, buying_power, pdt):
        return CLIMessages._loader.get(
            "portfolio.account_info", equity=equity, cash=cash, buying_power=buying_power, pdt=pdt
        )
    ...
    def POSITION_DETAILS_HEADER(emoji, symbol):
        return CLIMessages._loader.get(
            "portfolio.position_details_header", emoji=emoji, symbol=symbol
        )
    ...
    def PRICE_TARGETS(tp, sl, tp_dist, sl_dist):
        return CLIMessages._loader.get(
            "portfolio.price_targets", tp=tp, sl=sl, tp_dist=tp_dist, sl_dist=sl_dist
        )
