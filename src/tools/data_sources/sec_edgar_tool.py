...
class SECEdgarTool:
    ...
    def __init__(self, download_dir: Optional[str] = None, use_temp_dir: bool = False):
        ...
        if use_temp_dir:
            ...
            self.logger.info(f"Using temporary directory for SEC filings: {self.download_dir}")
        else:
    ...
    def fetch_filings(self, 
        ...
        Args:
            ...
            before_date: Get filings before this date (format: YYYY-MM-DD)
            after_date: Get filings after this date (format: YYYY-MM-DD)
        ...
        try:
            ...
            if form_type not in FORM_TYPES and form_type not in FORM_TYPES.values():
                self.logger.warning(f"Form type '{form_type}' is not in the standard list.")
            
            # Process date parameters
            date_kwargs = {}
            if before_date:
                date_kwargs['before_date'] = before_date
            if after_date:
                date_kwargs['after_date'] = after_date
            
            # Download the filings
            self.logger.info(f"Downloading {num_filings} {form_type} filings for {ticker}")
            result = self.downloader.get(form_type, ticker, amount=num_filings, **date_kwargs)
            ...
            self.logger.error(f"Error fetching {form_type} filings for {ticker}: {e}")
            return pd.DataFrame()
        ...
        try:
            ...
            for file in files:
                ...
                for section, content in section_contents.items():
                    ...
                    record[f'{section}_length'] = len(content) if content else 0
                
                # Calculate metrics if we have risk factors
                if 'risk_factors_content' in record and record['risk_factors_content']:
                    metrics = self._calculate_metrics(record['risk_factors_content'])
    ...
    def _extract_filing_date(self, file_path: str, file_name: str) -> str:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read(10000)  # Read just the beginning to look for date
    ...
    def _extract_section(self, file_path: str, section_name: str) -> str:
        ...
        try:
            ...
            for marker in section_markers:
                pattern = re.compile(rf'(item\s*{marker.replace("item", "")}[\s.:]+.*?)(item\s*\d)', re.IGNORECASE | re.DOTALL)
            ...
            for marker in section_markers:
                ...
                if index >= 0:
                    ...
                    next_section_match = re.search(r'item\s*\d', text_content.lower()[index+len(pattern):])
                    if next_section_match:
                        end_pos = index + len(pattern) + next_section_match.start()
            ...
            self.logger.error(f"Error extracting {section_name} from {file_path}: {e}")
            return ""
    ...
    def _calculate_metrics(self, text: str) -> Dict[str, float]:
        ...
        try:
            ...
            if valid_sentences:
                total_words = sum(len(re.findall(r'\b\w+\b', s)) for s in valid_sentences)
                metrics['avg_sentence_length'] = total_words / len(valid_sentences)
            else:
            ...
            if words:
                ...
                metrics['complexity_score'] = avg_word_length * metrics['avg_sentence_length'] / 10
            else:
            ...
            if len(filings_df) >= 2:
                ...
                for metric in ['word_count', 'avg_sentence_length', 'complexity_score']:
                    if metric in filings_df.columns:
                        filings_df[f'{metric}_change'] = filings_df[metric].pct_change() * 100
                
                # Add percentage growth in text length
                content_col = f'{section}_length'
                if content_col in filings_df.columns:
                    filings_df[f'{section}_length_change'] = filings_df[content_col].pct_change() * 100
    ...
    def search_filings(self, 
        ...
        try:
            ...
            sections_to_extract = [section] if section else list(self.report_sections.keys())
            ...
            for _, filing in filings_df.iterrows():
                ...
                for term in search_terms:
                    ...
                    for match in matches:
                        ...
                        match_section = section if section else self._identify_section(content, match.start(), sections_to_search)
            ...
            if not results_df.empty and 'filing_date' in results_df.columns:
                results_df = results_df.sort_values(['filing_date', 'section'], ascending=[False, True])
    ...
    def clean_up(self, days_old: int = 7):
        ...
        try:
            ...
            self.logger.info(f"Cleaned up {count} files older than {days_old} days")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nExample 1: Fetch risk factors from latest 10-K")
        risk_factors = edgar_tool.fetch_filings("AAPL", "10-K", num_filings=1, 
        ...
        if not risk_comparison.empty:
            ...
            print(risk_comparison[['filing_date', 'word_count', 'word_count_change']])
        ...
        if not search_results.empty:
            ...
            for i, result in search_results.head(2).iterrows():
                print(f"Match in {result['section']} section: {result['context'][:100]}...")
