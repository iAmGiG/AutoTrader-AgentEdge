...
class FREDDataTool:
    ...
    def __init__(self, api_key: Optional[str] = None, verbose: bool = False):
        ...
        if api_key is None:
            ...
            if not api_key:
                ...
                raise ValueError("FRED API key is required. Add it to config.json under 'fredapi' key.")
    ...
    def get_series(self, series_id: str, 
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Could not fetch series info for {series_id}: {e}")
                df['Title'] = series_id
                df['Units'] = ''
                df['Frequency'] = ''
    ...
    def get_indicator(self, indicator: str,
        ...
        if not series_id:
            ...
            self.logger.info(f"Available indicators: {', '.join(ECONOMIC_INDICATORS.keys())}")
            return pd.DataFrame()
    ...
    def get_yield_curve(self, date: Optional[str] = None) -> pd.DataFrame:
        ...
        for maturity, series_id in maturities.items():
            try:
                ...
                self.logger.warning(f"Could not fetch data for {maturity} ({series_id}): {e}")
                yield_data[maturity] = None
    ...
    def get_indicator_comparison(self, 
        ...
        for indicator in indicators:
            ...
            if series_id:
                ...
                self.logger.warning(f"Unknown indicator: {indicator}, skipping")
        ...
        rename_cols = {col: f"{reverse_mapping.get(col, col)}" for col in df.columns if col != 'Date'}
    ...
    def list_available_indicators(self) -> pd.DataFrame:
        ...
        for name, series_id in ECONOMIC_INDICATORS.items():
            try:
                ...
                self.logger.warning(f"Could not fetch info for {name} ({series_id}): {e}")
                data.append({
                    'Name': name,
                    'Series ID': series_id,
