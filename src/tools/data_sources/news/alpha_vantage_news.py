        ...
        try:
            ...
            if "overall_sentiment_score" in news_df.columns:
                if min_sentiment > 0:
                    ...
                    filtered_df = news_df[news_df["overall_sentiment_score"] >= min_sentiment]
...
if __name__ == "__main__":
    ...
    # Fetch positive news
    positive_news = tool.fetch_news_by_sentiment("MSFT", min_sentiment=0.3)
    print("\nPositive news:")
    if not positive_news.empty:
        print(positive_news[["title", "overall_sentiment_score"]].head())
    else:
