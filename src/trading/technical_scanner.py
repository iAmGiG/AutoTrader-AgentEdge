...
class UsefulScanner:
    ...
    def get_raw_technical_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            price_change_5d = (current_price - close_prices.iloc[-6]) / close_prices.iloc[-6] * 100 if len(close_prices) >= 6 else 0
            price_change_20d = (current_price - close_prices.iloc[-21]) / close_prices.iloc[-21] * 100 if len(close_prices) >= 21 else 0
    ...
    def generate_useful_report(self, results: Dict[str, Any]) -> str:
        ...
        if results['watch_list']:
            ...
            for watch in sorted(results['watch_list'], key=lambda x: x['vote_score'], reverse=True):
                lines.append(f"- **{watch['symbol']}**: {watch['vote_score']:.2f} - {watch['recommendation']}")
            lines.append("")
        
        # Top rejects with reasons
        if results['rejects']:
            top_rejects = sorted(results['rejects'], key=lambda x: x['vote_score'], reverse=True)[:5]
            lines.extend([
        ...
        if results['opportunities']:
            ...
            lines.append(f"1. **Consider {top_opp['symbol']}** - Highest score ({top_opp['vote_score']:.2f})")
