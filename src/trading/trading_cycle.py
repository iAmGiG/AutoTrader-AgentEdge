...
class CostEfficientTradeCycle:
    ...
    def __init__(self, state_file: str = "state/cost_efficient_positions.json"):
        ...
        logger.info("CostEfficientTradeCycle initialized")
    ...
    def calculate_stop_adjustments(self, broker_state: Dict[str, Any]) -> List[StopAdjustment]:
        ...
        for symbol, broker_pos in broker_state["positions"].items():
            ...
            # Progressive stop adjustment logic
            new_stop = None
            reason = ""
    ...
    def batch_modify_orders(self, adjustments: List[StopAdjustment]) -> Dict[str, Any]:
        """
        Batch modify stop orders. Try to minimize API calls.
        """
        if not adjustments:
        ...
        for adjustment in adjustments:
            try:
                ...
                if success:
                    ...
                    logger.info(f"Adjusted {adjustment.symbol} stop: "
                               f"${adjustment.current_stop:.2f} → ${adjustment.new_stop:.2f}")
                else:
                    results["errors"].append(f"Failed to modify {adjustment.symbol} stop order")
    ...
    def evening_routine(self) -> str:
        ...
        try:
            ...
            report = self.generate_routine_report(
                RoutineType.EVENING, broker_state, discrepancies, []
            )
