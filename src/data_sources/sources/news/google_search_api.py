...
class GoogleSearchNewsCache:
    ...
    def normalize_title_for_dedup(self, title):
        ...
        title = re.sub(
            r"^(BREAKING:|UPDATE:|EXCLUSIVE:|CORRECTING AND REPLACING)[\s/]*",
            "",
            title,
            flags=re.IGNORECASE,
        )
        title = re.sub(
            r"\s*[-–—]\s*(MarketWatch|WSJ|Bloomberg|Reuters|CNBC|Barrons).*$",
            "",
            title,
            flags=re.IGNORECASE,
        )
    ...
    def _search_nearby_dates(
        self, ticker: str, target_date: str, end_date: str
...
class GoogleSearchNewsTool:
    ...
    def __init__(self, api_key: str = None, search_engine_id: str = None):
        ...
        self.api_key = (
            api_key
            or config_loader.get("GOOGLE_SEARCH_API_KEY")
            or os.getenv("GOOGLE_SEARCH_API_KEY")
        )
        self.search_engine_id = (
            search_engine_id
            or config_loader.get("GOOGLE_SEARCH_ENGINE_ID")
    ...
    def filter_results_by_context(self, results_df: pd.DataFrame, ticker: str) -> pd.DataFrame:
        ...
        for _, article in results_df.iterrows():
            ...
            has_negative_content = any(
                neg_filter.replace("-", "").lower() in text for neg_filter in negative_filters
            )
            ...
            if has_required_context and not has_negative_content:
                ...
                ticker_upper == "SPY"
                and any(term in text for term in ["s&p", "etf", "fund", "index"])
                and not has_negative_content
            ):
    ...
    def build_search_query(
        self, ticker: str, start_date: str, end_date: str, source_sites: List[str] = None
    ) -> str:
        ...
        if start_dt.year == 2022:
            ...
            for dt in [start_dt, end_dt]:
                ...
                date_terms.extend([f'"{month_name} {year}"', f'"{month_abbr} {year}"'])
    ...
    def search_historical_news(
        self, ticker: str, start_date: str, end_date: str, max_results: int = 10
    ) -> pd.DataFrame:
        """
    ...
    def _search_with_url_patterns(
        self, ticker: str, start_date: str, end_date: str, max_results: int
    ) -> pd.DataFrame:
        """
        ...
        source_queries = [
            ...
            # YYYYMM format
            (
                f"site:businesswire.com {enhanced_ticker} {year_month.replace('-', '')}",
                "BusinessWire",
            ),
        ]
        ...
        if all_results:
            ...
            df = df.sort_values(
                ["relevance_score", "published_date"], ascending=[False, False], na_position="last"
            )
...
def search_google_historical_news(
    ticker: str, start_date: str, end_date: str, max_results: int = 10
) -> pd.DataFrame:
    """
...
__all__ = ["GoogleSearchNewsTool", "search_google_historical_news", "google_search_news_tool"]
