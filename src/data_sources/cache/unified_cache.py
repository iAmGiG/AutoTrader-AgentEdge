...
class UnifiedCacheManager:
    ...
    **DEPRECATED**: This file-based cache is deprecated in favor of TradingCacheManager (SQLite).
    ...
    def _get_news_cache_key(
        self, query: str, start: str, end: str, source: str = "google_search"
    ) -> str:
    ...
    def get_market_data(
        ...
        if cache_path.exists():
            try:
                ...
                if "metadata" in cache_data:
                    ...
                    expires_at_value = (
                        cache_data["metadata"].get("expires_at") if expires_at_key else None
                    )
                else:
        ...
        if complete_result is not None:
            ...
            expected_trading_days = (
                calendar_days * 0.7
            )  # Rough estimate: ~70% of calendar days are trading days
        ...
        if (
            ...
            and start == f"{start[:4]}-01-01"
            and end == f"{end[:4]}-12-31"
        ):
    ...
    def _search_overlapping_cache(
        ...
        try:
            ...
            matching_files = list(self.market_dir.glob(pattern1)) + list(
                self.market_dir.glob(pattern2)
            )
            ...
            if overlapping_data:
                ...
                if quarterly_files:
                    ...
                    if quarterly_dfs:
                        ...
                        combined_quarterly = combined_quarterly[
                            ~combined_quarterly.index.duplicated(keep="first")
                        ]
    ...
    def set_news(
        self, query: str, start: str, end: str, data: pd.DataFrame, source: str = "google_search"
    ):
