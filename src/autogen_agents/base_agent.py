...
from ..data_sources.tools import ALL_TOOLS, fetch_unified_market_data
    ...
    def _parse_tool_arguments(self, tool_args: Any) -> Dict[str, Any]:
        ...
        if isinstance(tool_args, dict):
            ...
            try:
                ...
                self.log("Warning: Failed to parse tool arguments as JSON")
                return {}
        else:
            # Unknown format
                        ...
                        content_str = json.dumps(
                            {
                                ...
                                "error_info": (
                                    result.to_dict("records") if not result.empty else None
                                ),
                            }
                        )
                    else:
