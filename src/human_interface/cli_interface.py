...
class TradingCLI:
    ...
    def scan_market(self, symbols: Optional[List[str]] = None, 
                   execute_approved: bool = False) -> None:
        """
        ...
        try:
            ...
            signal_count = sum(1 for r in results['trading_recommendations'].values() 
    ...
    def _prompt_for_executions(self, validated_trades: Dict[str, Any]) -> None:
        ...
        if approved_trades:
            ...
            for symbol, validation_result in approved_trades:
                print(f"\nExecuting {symbol}...")
                execution_result = self.orchestrator.execute_approved_trade(symbol, validation_result)
                
                if execution_result.get('execution_result', {}).get('status') == 'FILLED':
                    print(f"✅ {symbol} trade executed successfully")
                else:
                    error = execution_result.get('error') or execution_result.get('execution_result', {}).get('reason', 'Unknown error')
...
def main():
    ...
    scan_parser.add_argument('--execute', action='store_true', 
                           help='Prompt for trade execution')
