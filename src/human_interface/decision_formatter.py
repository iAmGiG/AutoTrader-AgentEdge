...
class DecisionFormatter:
    ...
    def format_trading_decision(
        ...
        for symbol, trade_info in validated_trades.items():
            ...
            # Expected outcomes
            # Load exit percentages from config
            config = TradingConfig()
            take_profit_pct = config.get_risk_config("take_profit")
            stop_loss_pct = config.get_risk_config("stop_loss")
    ...
    def format_position_summary(
        self, positions: List[Dict[str, Any]], account_status: Dict[str, Any]
    ) -> str:
        """
    ...
    def format_exit_recommendation(self, symbol: str, exit_analysis: Dict[str, Any]) -> str:
        ...
        # Decision emoji
        decision_emoji = {"EXIT_PROFIT": "🎯", "EXIT_LOSS": "🛑", "HOLD": "⏳", "ERROR": "❌"}.get(
            decision, "❓"
        )
    ...
    def format_execution_result(self, execution_results: List[Dict[str, Any]]) -> str:
        ...
        summary_parts = ["🔄 EXECUTION RESULTS", "=" * 30, ""]
        ...
        for result in execution_results:
            ...
            if status == "FILLED":
                ...
                summary_parts.extend([f"❌ {symbol}: FAILED", f"   Reason: {reason}", ""])
