...
class MetricsCapture:
    ...
    def record_trade(self, date: str, action: str, price: float, shares: int, 
        ...
        if action == 'BUY':
            ...
            self.update_sentiment_effectiveness(self.position_entry_sentiment, return_pct, trade_record)
    ...
    def export_enhanced_results(self) -> Dict:
        ...
        # Trade statistics
        completed_trades = [t for t in self.trade_data if t['action'] == 'SELL' and 'return_pct' in t]
        win_rate = sum(1 for t in completed_trades if t['return_pct'] > 0) / len(completed_trades) * 100 if completed_trades else 0
        avg_holding_period = np.mean([t['holding_period_days'] for t in completed_trades if 'holding_period_days' in t]) if completed_trades else 0
    ...
    def get_regime_summary(self) -> Dict:
        ...
        for regime, data_list in regime_summary.items():
            if data_list:
                ...
                summary[regime] = {'days': 0, 'avg_daily_return': 0, 'total_return': 0, 'volatility': 0}
        
        return summary


# Integration helper functions for existing backtesting framework

def integrate_with_backtest(backtest_instance, metrics_capture: MetricsCapture):
    """Helper function to integrate MetricsCapture with existing backtest framework."""
    # This would be called from the main backtesting loop
    # Example integration points:
    
    def enhanced_daily_update(date, portfolio_value, cash, position, stock_price, sentiment, **kwargs):
        """Enhanced daily update with metrics capture."""
        # Call original daily update logic
        # Then update metrics capture
        metrics_capture.update_daily_data(
            ...
            macd_histogram=kwargs.get('macd_histogram')
        )
    
    def enhanced_trade_execution(date, action, price, shares, sentiment, macd_signal, portfolio_value):
        """Enhanced trade execution with metrics capture."""
        # Call original trade execution logic
        # Then record in metrics capture
    ...
    return enhanced_daily_update, enhanced_trade_execution


def create_enhanced_backtest_results(metrics_capture: MetricsCapture, original_results: Dict) -> Dict:
    """Merge enhanced metrics with original backtest results."""
    enhanced_data = metrics_capture.export_enhanced_results()
    
    # Merge with original results while preserving existing structure
    enhanced_results = original_results.copy()
    
    # Add enhanced sections
    enhanced_results['enhanced_metrics'] = enhanced_data['enhanced_daily_data']
    enhanced_results['enhanced_trades'] = enhanced_data['enhanced_trade_data']
    enhanced_results['sentiment_effectiveness'] = enhanced_data['sentiment_effectiveness']
    enhanced_results['market_regime_analysis'] = enhanced_data['market_regime_summary']
    enhanced_results['real_time_metrics'] = enhanced_data['real_time_metrics']
    
    # Update metadata to indicate enhanced capture
    if 'metadata' not in enhanced_results:
        enhanced_results['metadata'] = {}
    enhanced_results['metadata']['enhanced_capture'] = True
    enhanced_results['metadata']['capture_framework'] = 'MetricsCapture_v1.0'
