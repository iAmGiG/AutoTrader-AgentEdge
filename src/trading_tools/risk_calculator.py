...
def calculate_portfolio_risk(positions: List[Dict], cash: float, 
    ...
    # Position exposure
    position_exposure_pct = (total_position_value / total_portfolio_value) * 100 if total_portfolio_value > 0 else 0
    ...
    total_unrealized_pnl_pct = (total_unrealized_pnl / total_portfolio_value) * 100 if total_portfolio_value > 0 else 0
...
def check_portfolio_limits(current_positions: int, position_exposure_pct: float,
    ...
    limits = {
        ...
        'diversification_ok': current_positions <= 1 or largest_position_pct <= 25  # If only 1 position, allow up to 25%
    }
