...
class DetailedBacktest:
        ...
        for i in range(len(prices)):
            ...
            if position is None and entry_signals.iloc[i] and i < len(prices) - 1:
                ...
                if exit_reason or i == len(prices) - 1:
                    ...
                    trade_return = (exit_value - position['capital_invested']) / position['capital_invested']
    ...
    def _analyze_results(self, trades: List[Dict], portfolio_value: List[float], prices: pd.Series) -> Dict:
...
def main():
    ...
    for take_profit, stop_loss, name in configs:
        ...
        expected = win_rate * results['avg_win_per_trade'] + (1 - win_rate) * results['avg_loss_per_trade']
        print(f"   Win rate × Avg win: {win_rate:.3f} × {results['avg_win_per_trade']:.2f}% = {win_rate * results['avg_win_per_trade']:.2f}%")
        print(f"   Loss rate × Avg loss: {1-win_rate:.3f} × {results['avg_loss_per_trade']:.2f}% = {(1-win_rate) * results['avg_loss_per_trade']:.2f}%")
        ...
        if results['total_trades'] > 0:
            ...
            for i, trade in enumerate(results['trades'][:3]):
                print(f"   Trade {i+1}: ${trade['capital_before']:.2f} → ${trade['capital_after']:.2f}")
                print(f"            Return: {trade['per_trade_return']:+.2f}% in {trade['days_held']} days ({trade['exit_reason']})")
    
    # Buy and hold comparison
    buy_hold_return = (prices.iloc[-1] / prices.iloc[0] - 1) * 100
    print(f"\n📊 BUY & HOLD BENCHMARK:")
    print(f"   Return: {buy_hold_return:.2f}%")
    print(f"   No trades, no stops, just hold")
