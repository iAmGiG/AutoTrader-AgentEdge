...
class BacktestRunner:
    ...
    def run_suite(self, suite_name: str, parallel: Optional[bool] = None, 
        ...
        summary_file = self.results_dir / f"suite_summary_{suite_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def list_suites(self):
        ...
        for suite_name, suite_config in self.config['test_suites'].items():
            ...
            print(f"  Timeout: {suite_config['timeout_minutes']} minutes per test")
...
Examples:
  ...
  python run_backtest_suite.py --resume                 # Resume interrupted run
    ...
    try:
        ...
        if not args.suite:
            print("Error: Please specify a test suite or use --list to see available suites.")
            parser.print_help()
            return
