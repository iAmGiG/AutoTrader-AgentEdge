...
def collect_benchmark_data():
    ...
    logger.info(f"📅 Collecting data for {len(symbols)} symbols × {len(quarters)} quarters = {len(symbols) * len(quarters)} total calls")
    ...
    for symbol, description in symbols:
        ...
        for i, (start_date, end_date) in enumerate(quarters, 1):
            call_count += 1
            logger.info(f"📊 [{call_count}/{total_calls}] Fetching {symbol}: {start_date} to {end_date}")
            
            try:
                result = av_tool.fetch_stock_data(symbol, start_date, end_date)
                
                if result is not None and not result.empty:
                    logger.info(f"✅ Success: {symbol} {start_date} to {end_date} ({len(result)} records)")
                    successful += 1
                else:
