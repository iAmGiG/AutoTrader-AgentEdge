...
def main() -> None:
    ...
    if prices is None or prices.empty:
        ...
        print(f"Cache miss - fetching market data for {symbol} from {start} to {end}")
    ...
    for i, (ts, price) in enumerate(prices.items()):
        ...
        try:
            ...
            if not sigs.get('ok', False):
                print(f"⚠️  Skipping {date_str}: Error in signals - {sigs.get('error', 'Unknown error')}")
                skipped_days += 1
                continue
            ...
            print(f"Signals received: sentiment={sentiment_data}, technical={technical_data}")
            ...
            if decision.get("action") == "BUY" and equity >= price * qty:
                ...
                print(f"✅ Executed SELL: {shares} shares @ ${price:.2f} = ${sell_value:.2f}")
                shares = 0
    ...
    print(f"  Successful: {successful_days} ({successful_days/total_days*100:.1f}%)")
    ...
    if trades:
        ...
        # Save trades to CSV
        csv_filename = f"{symbol}_trades_{start}_{end}.csv"
    ...
    if equity_curve:
        ...
        equity_filename = f"{symbol}_equity_{start}_{end}.csv"
