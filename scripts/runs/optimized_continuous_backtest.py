#!/usr/bin/env python3
"""
Optimized Continuous Backtesting with Checkpoint/Resume

Uses optimized agents that bypass unnecessary LLM calls for better performance:
- V0: No LLM usage (baseline)
- V1: Direct Google Search tool access with batch processing
- V2: Direct VXX tool access with batch processing  
- V3: Direct combination of V1+V2 without LLM
- V4: Already optimized with weekly batch processing

Expected performance improvements:
- V1-V3: 80-90% fewer LLM calls (252 → 20-50 calls per year)
- Faster execution, same results quality
- Better suited for full-year continuous testing
"""
...
# Import OPTIMIZED sentiment agents
from src.agents_optimized.sentiment_v0 import V0SentimentAgent
...
class OptimizedContinuousBacktest:
    """
    Optimized continuous backtesting with direct tool access for V1-V3.

    Key Improvements:
    - Uses optimized agents that bypass unnecessary LLM calls
    - Batch processing for entire periods before daily trading loop
    - Same trading logic and results as original
    - Significantly faster execution
    """

    def __init__(self, output_dir: str = "reports/continuous_backtests_optimized"):
        ...
        # Initialize OPTIMIZED sentiment agents
        self.agents = {
            'V0': V0SentimentAgent(),
            'V1': OptimizedSentimentV1Agent(),
            'V2': OptimizedSentimentV2Agent(),
            'V3': OptimizedSentimentV3Agent(),
            'V4': OptimizedSentimentV4Agent(enable_date_sanitization=True)
        }

        logger.info("✅ Optimized Continuous Backtest initialized with optimized agents")
    ...
    async def get_sentiment(self, agent, symbol: str, date_str: str) -> float:
        """Get sentiment score from optimized agent."""
        try:
    ...
    async def run_continuous_backtest(self, version: str, symbol: str, year: int,
        ...
        """
        Run continuous backtest using OPTIMIZED agents.
        """

        logger.info(f"🚀 Starting OPTIMIZED continuous backtest: {version} {symbol} {year}")
        ...
        if checkpoint:
            ...
            if checkpoint.last_date == "batch_preparation_complete":
                # Batch preparation was completed, resume from start of daily trading
                resume_data = market_data
                logger.info(
                    f"🔄 Resuming from completed batch preparation ({len(resume_data)} days to trade)")
            else:
        ...
        # OPTIMIZATION: Batch preparation for V1-V4 (V0 doesn't need it)
        if version != 'V0' and (not checkpoint or checkpoint.last_date != "batch_preparation_complete"):
            logger.info(f"🤖 {version} Optimized: Preparing batch sentiment analysis...")
            try:
                # Use market data date range for batch preparation
                first_date = resume_data.index[0].strftime('%Y-%m-%d')
                last_date = resume_data.index[-1].strftime('%Y-%m-%d')

                if version == 'V4':
                    # V4 uses its existing weekly batch system
                ...
                if success:
                    logger.info(
                        f"✅ {version} Optimized: Batch preparation completed for {len(resume_data)} days")
                    ...
                    logger.info(f"💾 {version} Optimized: Batch preparation checkpoint saved")

                else:
                    logger.warning(
                        f"⚠️ {version} Optimized: Batch preparation failed, continuing with daily mode")

            except Exception as e:
                logger.error(f"❌ {version} Optimized: Batch preparation error: {e}")
                logger.info(f"🔄 {version} Optimized: Continuing with daily sentiment analysis")

        # Daily trading loop (same as original - just faster sentiment calls)
        for date, row in resume_data.iterrows():
            ...
            # Trading logic: MACD + Sentiment (SAME AS ORIGINAL)
            if position == 0 and macd_signal == 1 and sentiment_score >= 0:
                # Enter long position
                shares = int(cash / current_price)
                if shares > 0:
                    position = shares
                    cash = round(cash - (shares * current_price), 2)
                ...
                # Exit position
                cash = round(cash + (position * current_price), 2)
                ...
                trades.append({
                    ...
                    'avg_cost_return_pct': avg_cost_return,
                    'entry_date': entry_date,
                    'entry_avg_cost': position_avg_cost
                })

                logger.info(
                    f"📉 SELL: {position} shares at ${current_price:.2f} (return: {avg_cost_return:+.2f}%)")
            ...
            # Record daily portfolio value (SAME AS ORIGINAL)
        ...
        results = {
            'metadata': {
                ...
                'framework': 'OptimizedContinuousBacktest_v1.0',
                'optimization': 'Direct tool access, batch processing'
            },
            'performance': metrics,
            'trades': trades,
        ...
        logger.info(
            f"🎉 {version} {symbol} {year} completed: {metrics['total_return']:+.2f}% return (OPTIMIZED)")
    ...
    def _calculate_metrics(self, daily_values: List[Dict], trades: List[Dict],
        ...
        return {
            ...
            'avg_trade_return': round(np.mean([t.get('return_pct', 0) for t in trades if 'return_pct' in t]), 2) if trades else 0
        }
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Optimized Continuous Backtesting')
    ...
    backtest = OptimizedContinuousBacktest()

    if args.status:
        # Show status summary
        summary = backtest.get_status_summary()
        print("\n📊 OPTIMIZED BACKTEST STATUS SUMMARY")
        print("=" * 50)
    ...
    if args.all_versions:
        ...
        print(f"\n🚀 Running all OPTIMIZED versions: {period_desc}")
        print("=" * 60)

        for version in versions:
            try:
                print(f"\n📈 Starting {version} (optimized)...")
        ...
        # Print comparison
        print(f"\n📊 OPTIMIZED FINAL COMPARISON: {args.symbol} {args.year}")
        print("-" * 50)
        for version in versions:
        ...
        try:
            ...
            print(f"\n🎉 {args.version} {args.symbol} {args.year} completed (OPTIMIZED)!")
