...
def extract_metrics(results: Dict) -> Dict:
    ...
    if daily_values:
        unrealized_pnls = [day.get('unrealized_pnl_pct', 0)
                           for day in daily_values if day.get('position', 0) > 0]
...
def print_comparison_report():
    ...
    print("-" * 50)
    core_cols = ['Version', 'Strategy',
                 'Total Return (%)', 'Outperformance (%)', 'Total Trades', 'Win Rate (%)']
    print(df[core_cols].to_string(index=False))

    # Risk-Adjusted Returns
    print("\n📈 RISK-ADJUSTED RETURNS")
    print("-" * 50)
    risk_cols = ['Version', 'Max Drawdown (%)', 'Volatility (%)',
                 'Sharpe Ratio', 'Time in Market (%)']
    print(df[risk_cols].to_string(index=False))

    # Trading Performance
    print("\n💼 TRADING PERFORMANCE")
    print("-" * 50)
    trade_cols = ['Version', 'Avg Trade Return (%)',
                  'Best Trade (%)', 'Worst Trade (%)', 'Avg Hold Days']
    ...
    print(
        f"✅ Best Total Return: {best_return['Version']} ({best_return['Strategy']}) with {best_return['Total Return (%)']:.2f}%")

    # Most consistent
    if not df_valid_sharpe.empty:
        best_sharpe = df_valid_sharpe.loc[df_valid_sharpe['Sharpe Ratio'].idxmax()]
        print(
            f"✅ Best Risk-Adjusted: {best_sharpe['Version']} with Sharpe ratio of {best_sharpe['Sharpe Ratio']:.3f}")
