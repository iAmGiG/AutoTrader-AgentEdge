...
def generate_markdown_report(results: Dict[str, Dict[str, Dict]], df: pd.DataFrame) -> str:
    ...
    report.append("**Test Period**: 2024 Full Year")
    report.append("**Framework**: Simple Continuous Backtest v1.0")
    ...
    for ticker in df["Ticker"]:
        has_v4 = (
            "V4_Return" in df.columns and df[df["Ticker"] == ticker]["V4_Return"].iloc[0] != "N/A"
        )
        if has_v4:
    ...
    report.append("**V2 (Market Fear)**: VXX volatility-based fear/complacency sentiment")
    report.append("**V3 (Combined)**: Heuristic combination of V1 news + V2 VXX signals")
    report.append("**V4 (LLM Reasoning)**: GPT-4 intelligent sentiment with date sanitization")
    ...
    report.append("- **Position Sizing**: Sentiment-based scaling (30%-100% of available cash)")
    report.append(
    ...
    if execution_cost_analysis:
        ...
        if cost_impact_by_version:
            ...
            for version, data in sorted(cost_impact_by_version.items()):
                ...
                print(
                    f"  • {version}: {avg_cost_drag:.2f}% drag, {avg_trades:.1f} trades/strategy, ${avg_cost:.2f} total cost"
                )
...
def main():
    ...
    parser.add_argument("--basic", action="store_true", help="Generate basic summary (default)")
    ...
    if ADVANCED_AVAILABLE:
        ...
        print("   • Sentiment effectiveness analysis")
        print("   • Risk-adjusted performance (Sharpe, Calmar ratios)")
        print("   • Market regime analysis")
        print("   • Trade quality metrics")
        print("   • Execution cost modeling and analysis")
        print("   • Cost efficiency rankings and recommendations")
