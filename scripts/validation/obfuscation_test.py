...
async def run_obfuscation_test(
    symbol: str = "AAPL",
...
Examples:
  ...
  python scripts/validation/obfuscation_test.py --quiet            # Minimal output
    ...
    parser.add_argument("--start", default="2024-01-15", help="Start date (default: 2024-01-15)")
    parser.add_argument("--end", default="2024-01-19", help="End date (default: 2024-01-19)")
    ...
    results = asyncio.run(
        run_obfuscation_test(
            symbol=args.symbol, start_date=args.start, end_date=args.end, verbose=not args.quiet
        )
    )
