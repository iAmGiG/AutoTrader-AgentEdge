...
class FourIndicatorStrategy:
    ...
    def generate_signals(self, prices: pd.Series) -> pd.Series:
        ...
        for i in range(50, len(prices)):  # Start after indicators have enough data
            ...
            buy_votes = sum(1 for sig in [macd_signal, rsi_signal, bb_signal, stoch_signal] if sig.signal == 1)
            sell_votes = sum(1 for sig in [macd_signal, rsi_signal, bb_signal, stoch_signal] if sig.signal == -1)
    ...
    if results:
        ...
        for threshold in thresholds:
            if threshold in results:
                ...
                print(f"   {threshold}/4: {results[threshold]['num_trades']} trades ({trades_per_month:.1f}/month)")
