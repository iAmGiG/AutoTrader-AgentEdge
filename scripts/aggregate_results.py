...
class BacktestAggregator:
    ...
    def calculate_aggregate_statistics(self) -> Dict:
        ...
        for (symbol, dates), metrics_df in self.metrics_data.items():
            ...
            stats["worst_drawdown"] = max(stats["worst_drawdown"], metrics.get("max_drawdown_pct", 0))
    ...
    def generate_summary_report(self, output_path: str = None) -> str:
        ...
        report.append(f"Success Rate:     {stats['profitable_tests']}/{stats['total_tests']} ({stats['profitable_tests']/stats['total_tests']*100:.1f}%)")
        report.append("```")
        ...
        report.append(f"- **Profitable Tests**: {stats['profitable_tests']}/{stats['total_tests']} ({stats['profitable_tests']/stats['total_tests']*100:.1f}%)")
        ...
        report.append(f"\n### Best Performer")
        report.append(f"{best_emoji} **{stats['best_performing_stock']}**: {stats['max_return']:.2f}%")
        
        report.append(f"\n### Worst Performer")
        report.append(f"{worst_emoji} **{stats['worst_performing_stock']}**: {stats['min_return']:.2f}%")
        ...
        tests_with_trades = sum(1 for m in self.metrics_data.values() if not m.empty and m.iloc[0].get('num_trades', 0) > 0)
        trade_frequency = tests_with_trades / stats['total_tests'] * 100 if stats['total_tests'] > 0 else 0
        
        report.append("\n### Strategy Observations")
        
        if trade_frequency < 25:
            report.append("- 🔴 **Low Trade Frequency**: Only " + f"{trade_frequency:.0f}% of tests generated trades")
        ...
        if stats["total_trades"] > 0:
            report.append(f"- **Average Trades per Test**: {stats['total_trades'] / stats['total_tests']:.1f}")
        else:
            report.append("- **Average Trades per Test**: 0")
        
        report.append(f"- **Tests with Trades**: {tests_with_trades}/{stats['total_tests']} ({trade_frequency:.0f}%)")
        report.append(f"- **Total Market Days Analyzed**: {stats['total_signals_generated']}")
        report.append(f"- **Average Test Duration**: {stats['total_signals_generated']/stats['total_tests']:.1f} days")
        report.append("</details>")
