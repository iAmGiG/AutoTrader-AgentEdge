...
def generate_architecture_diagram():
    ...
    with dot.subgraph(name='cluster_data') as c:
        ...
        c.node('alpaca', 'Alpaca Markets\n(Trading & Market Data)',
               fillcolor=DATA_SOURCE, fontcolor='white')
    ...
    with dot.subgraph(name='cluster_infra') as c:
        ...
        c.node('cache', 'SQLite Cache\n(8-10x Performance)',
               fillcolor=INFRASTRUCTURE, fontcolor='white')
        c.node('config', 'YAML Configuration\n(Dynamic Parameters)',
               fillcolor=INFRASTRUCTURE, fontcolor='white')
        c.node('tools', 'AutoGen Tools\n(Market Data & Indicators)',
               fillcolor=INFRASTRUCTURE, fontcolor='white')

    # AutoGen Agent Layer
    with dot.subgraph(name='cluster_agents') as c:
        c.attr(label='AgentEdge Multi-Agent System', style='filled,bold',
               color='#1976D2', fontsize='14', margin='20')

        # Production-ready agent
        c.node('voter', 'VoterAgent ✓\nMACD+RSI Voting\n(0.856 Sharpe)',
               fillcolor=PRODUCTION, fontcolor='white', penwidth='3')

        # Agents in development
        c.node('scanner', 'ScannerAgent\nMarket Scanning\n(In Development)',
               fillcolor=DEVELOPMENT, fontcolor='white', style='rounded,filled,dashed')
        c.node('risk', 'RiskAgent\nPosition Sizing\n(In Development)',
               fillcolor=DEVELOPMENT, fontcolor='white', style='rounded,filled,dashed')
        c.node('executor', 'ExecutorAgent\nTrade Execution\n(In Development)',
               fillcolor=DEVELOPMENT, fontcolor='white', style='rounded,filled,dashed')

    # Orchestration Layer
    dot.node('orchestrator', 'Trading Orchestrator\n(Multi-Agent Coordination)',
             shape='hexagon', fillcolor='#6A1B9A', fontcolor='white',
    ...
    with dot.subgraph(name='cluster_ui') as c:
        ...
        c.node('cli', 'Interactive CLI\n(Approve → Auto-Manage)',
               shape='rectangle', fillcolor='#C62828', fontcolor='white',
               fontsize='12', penwidth='2')
        c.node('approval', 'Human Approval Gate\n(Initial Trade Decision)',
               shape='diamond', fillcolor='#F57C00', fontcolor='white',
    ...
    with dot.subgraph(name='cluster_legend') as c:
        ...
        c.node('leg_prod', '✓ Production Ready',
               fillcolor=PRODUCTION, fontcolor='white', shape='box', style='rounded,filled')
        c.node('leg_dev', 'In Development',
               fillcolor=DEVELOPMENT, fontcolor='white', shape='box', style='rounded,filled,dashed')
        c.attr(rank='same')
