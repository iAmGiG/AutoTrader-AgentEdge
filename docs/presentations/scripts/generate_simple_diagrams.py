...
def generate_architecture():
    ...
    add_box(ax, 2, 1, 6, 0.7, 'Human Approval Gate\n0.856 Sharpe Performance',
            AMBER, 'black', fontsize=16)

    output_dir = os.path.join(os.path.dirname(__file__), 'figures')
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, '1_architecture.png'),
...
def generate_voting():
    ...
    ax.text(5, 0.3, 'Both Agree → Strong | One Signals → Weak | Conflict → Hold',
            ha='center', fontsize=18, style='italic')

    output_dir = os.path.join(os.path.dirname(__file__), 'figures')
    plt.savefig(os.path.join(output_dir, '2_voting_logic.png'),
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def generate_performance_simple():
    ...
    ax.text(10, 9.5, 'Validated Performance: Voting vs Single Indicator',
    ...
    add_box(ax, 4, 1.5, 12, 0.9, 'Extended Testing: 11.2% better in volatile markets', GREEN, fontsize=11)

    output_dir = os.path.join(os.path.dirname(__file__), 'figures')
    plt.savefig(os.path.join(output_dir, '3_performance.png'),
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def generate_workflow():
    ...
    output_dir = os.path.join(os.path.dirname(__file__), 'figures')
    plt.savefig(os.path.join(output_dir, '4_workflow.png'),
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def generate_metrics():
    ...
    ax.text(5, 1.0, '11.2% Better in Volatile Markets', ha='center',
            fontsize=16, color=GREEN, fontweight='bold')

    output_dir = os.path.join(os.path.dirname(__file__), 'figures')
    plt.savefig(os.path.join(output_dir, '5_metrics.png'),
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def generate_stack():
    ...
    ax.text(5, 1, 'Production-Ready • Open Source (AGPL-3.0)', ha='center',
            fontsize=10, fontweight='bold')

    output_dir = os.path.join(os.path.dirname(__file__), 'figures')
    plt.savefig(os.path.join(output_dir, '6_stack.png'),
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
